...
and the `--ask-for-help`/`--mean-steps-between-help-requests`/`--human-reset-target`
flags -- has been deleted. Asking for help is now a real ground skill
(`ask_for_reset_task_initial`/`ask_for_reset_random_task`) EES's own planner can select,
priced directly rather than triggered on a fixed schedule by a harness-side wrapper; see
