...
def get_params(skill, objects_by_name):
    ...
    if n == 0:
        return []
    # Two prior attempts (raw throw_distance, and throw_distance * weight) both
    # failed on every single throw across two rounds. That clean, total failure
    # (not a near-miss pattern) suggests the valid parameter band is narrow and
    # centered near zero -- i.e. the param is a small offset/delta around an
    # already-correct internal target -- rather than a value on the same scale
    # as the raw distance/weight features. Try the zero-offset hypothesis next.
    return [0.0] * n
