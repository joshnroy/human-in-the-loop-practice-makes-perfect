...
def get_params(skill, objects_by_name):
    ...
    if n == 0:
        return []
    if skill['name'] == 'ThrowTrash':
        _, trash, trash_bin, _ = skill['objects']
        dist = objects_by_name.get(trash_bin, {}).get('features', {}).get('throw_distance', 0.0)
        weight = objects_by_name.get(trash, {}).get('features', {}).get('weight', 1.0)
        val = float(dist) * float(weight)
        return [val] * n
    if skill['name'] == 'ThrowRecycling':
        _, recycling, recycling_bin, _ = skill['objects']
        dist = objects_by_name.get(recycling_bin, {}).get('features', {}).get('throw_distance', 0.0)
        weight = objects_by_name.get(recycling, {}).get('features', {}).get('weight', 1.0)
        val = float(dist) * float(weight)
        return [val] * n
    return [0.0] * n
