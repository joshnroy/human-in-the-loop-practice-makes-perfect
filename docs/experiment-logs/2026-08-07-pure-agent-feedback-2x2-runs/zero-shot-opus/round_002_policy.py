...
_S = {
    ...
    "base": 0,        # sweep offset, bumped once per episode
...
def policy(observation):
    ...
    if new_task:
        _S["base"] = (_S["base"] + max(1, _S["tries"])) % _NCAND
        _S["tries"] = 0
        _S["visits"] = {}
        _S["dry"] = set()
        _S["pk0"] = pk
    ...
    def hypothesis(skill):
        ...
        if h is not None:
            return h
        h = (_S["base"] + seen) % _NCAND
        for _ in range(_NCAND):           # skip falsified hypotheses
