...
Sweep progression is driven by the falsified index/value sets, which are updated
as soon as a throw resolves.  Scanning from index 0 therefore always lands on the
first hypothesis not yet ruled out, within an episode and across episodes alike.
Any note about the *current* episode (a spent pile, room visit counts) expires,
so stale evidence can never veto an action for the rest of the run.
"""
