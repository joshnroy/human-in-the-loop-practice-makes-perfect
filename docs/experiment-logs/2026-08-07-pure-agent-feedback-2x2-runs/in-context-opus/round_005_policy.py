...
_ORDER = (
    # Top cluster: every entry sits inside the confirmed-good band, so being
    # pushed a few slots down the list still throws something that lands.
    ("w", 0.5375), ("w", 0.530), ("w", 0.545), ("w", 0.520),
    ("w", 0.555), ("w", 0.515), ("w", 0.560), ("w", 0.535),
    ("p", (0.521, 1.176)),
    ("w", 0.575), ("p", (0.485, 1.44)), ("w", 0.50),
...
def _prior(i):
    ...
    """
    if i < 8:  # the confirmed-good cluster
        return (6.0, 6.0)
    return (1.5, 2.0)
...
def _policy(observation):
    ...
    def _feasible(start, waypoints):
        """Can the robot still visit each waypoint in order?  Movement is
        one-way through rooms with blocks_right, so order matters."""
        cur = start
        for room in waypoints:
            if room is None:
                return False
            if room != cur and room not in _distances(graph, cur):
                return False
            cur = room
        return True
    ...
    def _dispose(item, is_trash):
        ...
        # Bin full: press the matching button before throwing.
        if (empty_pred, (target_bin,)) not in atom_set:
            ...
            if i is not None:
                return _result(skills, i)
            # Doors are one-way, so a detour to the button can strand us away
            # from the bin.  Only detour if the bin survives the trip.
            rooms = sorted(a[1] for a in by_pred.get(button_pred, ()))
            detour = [r for r in rooms if _feasible(room_of_robot, [r, bin_room])]
            if detour:
                dist = _distances(graph, room_of_robot)
                detour.sort(key=lambda r: (dist.get(r, 10 ** 6), r))
                act = _move_toward(detour[0])
                if act is not None:
                    return act
            elif rooms and not _feasible(room_of_robot, [bin_room, rooms[0], bin_room]):
                act = _move_toward(rooms[0])  # bin is a dead end either way
                if act is not None:
                    return act
    ...
    if wanted:
        ...
        # Fetch the item before emptying the bin.  Rooms with blocks_right are
        # one-way, so a detour to the button can put the pile permanently out
        # of reach -- that is how a practice period was lost, pressing the
        # button and then never being able to pick anything up.  Press first
        # only when doing so is the ordering that keeps everything reachable.
        empty_pred = "TrashBinEmpty" if is_trash else "RecyclingBinEmpty"
        bin_pred = "TrashBinInRoom" if is_trash else "RecyclingBinInRoom"
        if (empty_pred, (target_bin,)) not in atom_set:
            press = "PressTrash" if is_trash else "PressRecycling"
            button_pred = (
                "TrashButtonInRoom" if is_trash else "RecyclingButtonInRoom"
            )
            bin_room = _room_of(bin_pred, target_bin)
            rooms = sorted(a[1] for a in by_pred.get(button_pred, ()))
            pickup_first_ok = any(
                _feasible(room_of_robot, [pile_room, r, bin_room]) for r in rooms
            )
            if not pickup_first_ok:
                press_first = [
                    r for r in rooms
                    if _feasible(room_of_robot, [r, pile_room, bin_room])
                ]
                if press_first:
                    i = _skill_named(
                        press, lambda o: len(o) > 3 and o[3] == target_bin
                    )
                    if i is None:
                        i = _any_named(press)
                    if i is not None:
                        return _result(skills, i)
                    act = _move_toward(press_first[0])
                    if act is not None:
                        return act
