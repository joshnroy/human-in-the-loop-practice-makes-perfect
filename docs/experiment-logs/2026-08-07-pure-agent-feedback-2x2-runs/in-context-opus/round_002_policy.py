...
def _prior(i):
    """Optimism for an untried hypothesis; the leading guesses start higher."""
    if i == 0:
        return 0.80
    return 0.78 if i < 4 else 0.75


def _value(mem, i):
    """Smoothed success rate.  One hit promotes, one miss demotes, and a
    hypothesis that keeps landing outscores the untried remainder."""
    return (mem["wins"][i] + 2.0 * _prior(i)) / (mem["tries"][i] + 2.0)
...
def _record_outcome(prev, atom_set):
    ...
    (practice showed a fresh pickup after every miss).  So an observation that
    still has us holding the object is not a miss, it is a call on a state the
    environment never stepped; charging those as misses is what burned four
    hypotheses before the first executed ThrowRecycling.
    """
...
def _policy(observation):
    ...
    if prev is not None:
        ...
        elif _record_outcome(prev, atom_set):
            _MEM["prev"].pop(ep, None)
        else:
