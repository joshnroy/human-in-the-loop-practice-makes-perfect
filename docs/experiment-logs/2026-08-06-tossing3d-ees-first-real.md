...
> **The oracle is no longer a ceiling arm, which bears on any comparison against it here.**
> It used to be one measured operating point (`standoff 1.35, 140 deg/s, 792 ms`) that
> scored `10/10` at its own standoff. It now draws all four continuous parameters from the
> fused controller's own sampler off a fixed `ORACLE_PARAMETER_SEED`. Measured on
> 2026-08-17 at the shipped seed it scores **0/10** scene seeds (125 and 0-8), resting the
> cube at x ~ 1.730 against a scoring window starting at x = 1.95. Sweeping the parameter
> seed on scene 125, **3/8** seeds score. So comparisons below that treat the oracle as an
> upper bound are comparing against something that no longer behaves that way.
