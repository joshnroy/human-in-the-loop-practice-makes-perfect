...
2. Full RRT / Multi-Stage Pick (clean vertical descent -> perfect center clamp -> clean
   toss into bin)
...
def simulate_full_episode(*, seed=103, mode="rrt"):
    ...
    for _ in range(40):
        ...
        state = env.observation_space.devectorize(obs)
        capture_frame(phase_name="1. Base Navigation to Standoff")
    ...
    if mode == "shortcut":
        ...
        for s in range(len(traj)):
            ...
            obs, _, _, _, _ = env.step(action)
            state = env.observation_space.devectorize(obs)
            capture_frame(phase_name="2. Arm Approach (1-Segment Shortcut)")
    else:
        for pt in dense_plan:
            ...
            obs, _, _, _, _ = env.step(action)
            state = env.observation_space.devectorize(obs)
            capture_frame(phase_name="2. Arm Approach (Full RRT Path)")
    ...
    for _ in range(25):
        ...
        obs, _, _, _, _ = env.step(action)
        state = env.observation_space.devectorize(obs)
        capture_frame(phase_name="3. Gripper Clamping")
    ...
    for _ in range(25):
        ...
        obs, _, _, _, _ = env.step(action)
        state = env.observation_space.devectorize(obs)
        capture_frame(phase_name="4. Lifting Cube")
    ...
    for _ in range(30):
        ...
        capture_frame(phase_name="5. Driving to Toss Standoff")
    ...
    for _ in range(20):
        ...
        capture_frame(phase_name="6. Arm Windup")
    ...
    for _ in range(25):
        ...
        capture_frame(phase_name="7. Ballistic Toss & Release")
    ...
    for _ in range(25):
        ...
        state = env.observation_space.devectorize(obs)
        capture_frame(phase_name="8. Flight into Bin")
...
def render_combined_video(*, seed=103, output_dir=Path("docs/rrt_analysis")):
    ...
    for i in range(total_frames):
        ...
        draw_wb.text(
            (10, 10), "📷 WIDE SCENE + BASE CYLINDER (r=0.25m)", fill=(0, 230, 255), font=font_small
        )

        draw_ca = ImageDraw.Draw(img_ca)
        draw_ca.rectangle([5, 5, 320, 30], fill=(0, 0, 0, 200))
        draw_ca.text(
            (10, 10), f"❌ SHORTCUT (Disp: {disp_a:.1f}mm)", fill=(255, 80, 80), font=font_med
        )

        draw_cb = ImageDraw.Draw(img_cb)
        draw_cb.rectangle([5, 5, 320, 30], fill=(0, 0, 0, 200))
        draw_cb.text(
            (10, 10), f"✅ FULL RRT (Disp: {disp_b:.1f}mm)", fill=(50, 255, 80), font=font_med
        )
        ...
        d.text(
            (20, 8),
            f"FULL PICK & TOSS ROLLOUT: (Left: Discarded Shortcut vs Center: Full RRT "
            f"Execution) | Step: {i:03d}",
            fill=(255, 255, 255),
            font=font_large,
        )
        ...
        d.text(
            (20, 360 + 58),
            "Cyan Wireframe = Mobile Base Cylinder (Radius 0.25m, Standoff 0.55m). Full RRT routes "
            "arm squarely around cube.",
            fill=(255, 230, 120),
            font=font_small,
        )
    ...
    print(
        f"\nSaved Full Grasp & Cylinder Analysis Video to {mp4_path} "
        f"({len(combined)} frames @ 10 fps)"
    )
