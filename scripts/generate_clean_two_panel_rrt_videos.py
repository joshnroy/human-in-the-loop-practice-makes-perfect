...
- Left Panel: Unobstructed Close-Up Cube Camera (elevated perspective, tracking grasp &
  displacement)
...
- Video 2: Multi-Stage Controller (With explicit standstill settle pauses at each waypoint,
  0.0 mm collision)
...
def render_two_panel_video(*, ctrl_class, is_baseline: bool, output_path: Path, seed: int = 103):
    ...
    def capture_frame(*, phase_title):
        ...
        if is_baseline:
            d.text(
                (20, 6),
                "VIDEO 1: BASELINE CONTROLLER (1-Segment Diagonal Descent) [1/2 SPEED]",
                fill=(255, 100, 100),
                font=font_large,
            )
            d.text(
                (20, 28),
                "Diagonal joint arc strikes cube top-right corner at Step 42 (27.2 mm "
                "displacement) -> Cube flings off-course",
                fill=(220, 180, 180),
                font=font_small,
            )
        else:
            d.text(
                (20, 6),
                "VIDEO 2: MULTI-STAGE WAYPOINT CONTROLLER (With Standstill Settle Stops) [1/2 "
                "SPEED]",
                fill=(100, 255, 100),
                font=font_large,
            )
            d.text(
                (20, 28),
                "1. Move to z+12cm & STOP -> 2. Align Wrist Yaw & STOP -> 3. Vertical Descent & "
                "STOP -> 4. Clamp & Toss",
                fill=(180, 220, 180),
                font=font_small,
            )
