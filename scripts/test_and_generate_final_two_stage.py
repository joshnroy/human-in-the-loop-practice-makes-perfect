"""Final, clean script to test and render two-panel photorealistic video on Seed 103:
- Right Panel: Original default camera view (task_view) with realistic textures & lighting
  (scene_bg=True)
- Left Panel: Elevated front-side close-up camera tracking grasp and displacement
- Two-Stage Controller:
  - Stage 1: Transit to z+10cm standoff above cube -> FULL STANDSTILL STOP & SETTLE (0.0 m/s)
  - Stage 2: Pure vertical elevator descent along Z -> FULL STANDSTILL STOP & SETTLE (0.0 m/s)
  - Stage 3: Clamp center of mass & vertical lift
  - Stage 4: Navigate to toss position -> Arm Windup -> High-Speed Toss into Goal Bin
...
class CleanTwoStagePickCubeController(TidyBotPickController):
    STANDOFF = (0.55, 0.0)
    ...
    def reset(self, x, params=None):  # noqa: PLR0917  (overrides TidyBotPickController.reset)
        ...
        for rotation in rotations:
            ...
            try:
                ...
                self._setup_trajectories(plan_x=upright, rotation=rotation)
                return
            except Exception:
                continue
        raise RuntimeError("Failed to plan clean two-stage pick")

    def _setup_trajectories(self, *, plan_x, rotation):
        ...
        # Pre-grasp pose: Strictly +10cm along World Z directly above the cube
        pre_grasp_pose = Pose(
            (
        ...
        # 2. Solve grasp WARM-STARTED from q_pre_raw to guarantee same branch!
        ...
        self._s2_traj, self._s2_dir = _compute_per_joint_profile(q_pre, q_grasp, v_desc, a_desc)
        ...
        # 1: Transit, 2: Pre-Grasp Settle (STOP), 3: Vertical Descent,
        # 4: Grasp Settle (STOP), 5: Retract
        self._stage = 1
    ...
    def current_stage_name(self) -> str:
        ...
        if self._stage == 1:
            return "Stage 1: Transit Above Cube (z+10cm)"
        if self._stage == 2:
            return "STOP & SETTLE above cube"
        if self._stage == 3:
            return "Stage 2: Pure Vertical Elevator Descent"
        if self._stage == 4:
            return "STOP & SETTLE around cube"
    ...
    def step(self):
        ...
        # Stage 4: FULL STOP at Grasp Pose
        if self._stage == 4:
            curr = np.array(self._get_current_robot_arm_conf()[:7])
...
def render_final_video(*, seed=103, output_dir=Path("docs/clean_two_stage_videos")):
    ...
    pick_ctrl = CleanTwoStagePickCubeController((robot, cube))
    ...
    skills = [
        ("1. Clean Two-Stage Pick", pick_ctrl, None, 200, []),
    ...
    def capture_frame(*, phase_title):
        ...
        # Right Panel: Original Default Camera View from env.render()
        w_raw = env.render()
        ...
        rc.cam.elevation = -14.0
        rc.cam.azimuth = 140.0
        ...
        d.text(
            (20, 6),
            "TWO-STAGE CONTROLLER: (Standoff Settle ──> Vertical Descent Settle ──> Toss) [1/2 "
            "SPEED]",
            fill=(100, 255, 100),
            font=font_large,
        )
        d.text(
            (20, 28),
            "1. Move to z+10cm & STOP -> 2. Pure vertical descent & STOP -> 3. Center Clamp & Toss",
            fill=(180, 220, 180),
            font=font_small,
        )
        d.text(
        ...
        d.text(
            (20, total_h - 35),
            "Photorealistic Lab2 Shading & Textures | Kinova Gen3 on TidyBot Base | Seed 103",
            fill=(255, 230, 120),
            font=font_small,
        )
    ...
    mp4_path = output_dir / f"clean_two_stage_seed{seed}.mp4"
    gif_path = output_dir / f"clean_two_stage_seed{seed}.gif"

    imageio.mimsave(mp4_path, frames, fps=10)
    imageio.mimsave(gif_path, frames[::2], fps=5)
    print(f"Saved Clean Two-Stage Video to {mp4_path} ({len(frames)} frames @ 10 fps)")
    return mp4_path, gif_path


if __name__ == "__main__":
    render_final_video(seed=103)
