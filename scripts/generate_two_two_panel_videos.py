"""Generate two separate dual-view videos (Left: Close-Up Cube View, Right: Global Wide
Scene View with Base Cylinder):
1. video_1_baseline_shortcut.mp4: Single-segment joint shortcut (strikes cube corner, flings cube).
2. video_2_full_rrt_plan.mp4: Full 31-waypoint RRT execution (clean vertical approach,
   0.0mm collision, clean toss into bin).
...
def draw_base_cylinder(
    *, draw, gl_cam, fovy_deg, base_pos, radius=0.25, height=0.45, w=640, h=480, color=(0, 230, 255)
):
    ...
    for a in angles:
        ...
        pb = project_point(
            gl_cam=gl_cam,
            fovy_deg=fovy_deg,
            world_point=np.array([px, py, 0.02]),
            width=w,
            height=h,
        )
        pt = project_point(
            gl_cam=gl_cam,
            fovy_deg=fovy_deg,
            world_point=np.array([px, py, height]),
            width=w,
            height=h,
        )
        if pb:
...
def generate_two_panel_video(*, mode="shortcut", seed=103, output_dir=Path("docs/dual_rrt_videos")):
    ...
    dense_plan = remap_joint_position_plan_to_constant_distance(
        rrt_plan, pyb.robot, max_distance=0.03
    )
    ...
    def record_frame(*, step_num, phase_title):
        ...
        draw_w.text(
            (16, 14), "📷 GLOBAL SCENE + BASE CYLINDER (r=0.25m)", fill=(0, 230, 255), font=font_tag
        )
        ...
        if mode == "shortcut":
            d.text(
                (20, 6),
                "BASELINE: 1-Segment Shortcut (Bypasses RRT Intermediate Waypoints) [1/2 SPEED]",
                fill=(255, 100, 100),
                font=font_large,
            )
            d.text(
                (20, 28),
                "Problem: Discards RRT collision-free waypoints -> single joint line sweeps finger "
                "into cube corner",
                fill=(220, 180, 180),
                font=font_small,
            )
            ...
            d.text(
                (20, 6),
                "FULL RRT EXECUTION: Following All 31 RRT Collision-Free Waypoints [1/2 SPEED]",
                fill=(100, 255, 100),
                font=font_large,
            )
            d.text(
                (20, 28),
                "Solution: Arm lifts high over cube and drops squarely from above -> Zero "
                "collision, clean toss",
                fill=(180, 220, 180),
                font=font_small,
            )
        ...
        d.text(
            (20, 46),
            f"Active Phase: {phase_title} | Step: {step_num:03d} | Seed: {seed}",
            fill=(80, 210, 255),
            font=font_med,
        )
        ...
        d.text(
            (20, total_h - 35),
            "Cyan Wireframe = Mobile Base Bounding Cylinder (Radius 0.25m, Standoff 0.55m).",
            fill=(255, 230, 120),
            font=font_small,
        )
    ...
    for _ in range(40):
        ...
        state = env.observation_space.devectorize(obs)
        record_frame(
            step_num=step_counter, phase_title="1. Mobile Base Navigation to Standoff (0.55m)"
        )
        step_counter += 1
    ...
    if mode == "shortcut":
        ...
        for s in range(len(traj)):
            ...
            record_frame(
                ...
                phase_title="2. Arm Descent (1-Segment Shortcut - Sweeps Corner)",
            )
            step_counter += 1
    else:
        for pt in dense_plan:
            ...
            state = env.observation_space.devectorize(obs)
            record_frame(
                step_num=step_counter, phase_title="2. Arm Descent (Full RRT Collision-Free Path)"
            )
            step_counter += 1
    ...
    for _ in range(25):
        ...
        obs, _, _, _, _ = env.step(action)
        state = env.observation_space.devectorize(obs)
        record_frame(step_num=step_counter, phase_title="3. Gripper Clamping on Cube")
        step_counter += 1
    ...
    for _ in range(25):
        ...
        obs, _, _, _, _ = env.step(action)
        state = env.observation_space.devectorize(obs)
        record_frame(step_num=step_counter, phase_title="4. Lifting Cube from Floor")
        step_counter += 1
    ...
    for _ in range(30):
        ...
        record_frame(step_num=step_counter, phase_title="5. Driving to Toss Location")
        step_counter += 1
        if move_ctrl.terminated():
            break
    ...
    for _ in range(20):
        ...
        record_frame(step_num=step_counter, phase_title="6. Arm Windup for Toss")
        step_counter += 1
        if windup_ctrl.terminated():
            break
    ...
    for _ in range(25):
        ...
        record_frame(step_num=step_counter, phase_title="7. Ballistic Toss & Release")
        step_counter += 1
        if toss_ctrl.terminated():
            break
    ...
    for _ in range(25):
        ...
        state = env.observation_space.devectorize(obs)
        record_frame(step_num=step_counter, phase_title="8. Flight & Settle in Bin")
        step_counter += 1
