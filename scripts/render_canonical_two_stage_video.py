...
class CanonicalTwoStagePickCubeController(TidyBotPickController):
    ...
    def reset(self, x, params=None):
        ...
        for rotation in rotations:
            ...
            try:
                ...
                self._setup_two_stages(upright, rotation)
                return
            except Exception:
                continue
    ...
    def _setup_two_stages(self, plan_x, rotation):
        ...
        s1_traj, s1_dir = _compute_per_joint_profile(q_home, q_pre, _ARM_MAX_VELOCITY, _ARM_MAX_ACCELERATION)
...
def render_canonical_videos(seed=103, output_dir=Path("docs/canonical_two_stage_videos")):
    ...
    def capture_frame(phase_title):
        ...
        d.text((20, 6), "TWO-STAGE PICK CONTROLLER: Aligned Standoff ──> Vertical Insertion [1/2 SPEED]", fill=(100, 255, 100), font=font_large)
        d.text((20, 28), "1. Direct transit to aligned standoff (z+10cm) & STOP -> 2. Straight vertical drop & STOP -> 3. Center Clamp & Toss", fill=(180, 220, 180), font=font_small)
