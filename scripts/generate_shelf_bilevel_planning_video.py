...
from kinder_bilevel_planning.env_models.dynamic3d.tidybot3d_shelf3D import create_bilevel_planning_models
...
def annotate_shelf_frame(closeup_raw, wide_raw, step, total_steps, current_op, current_skill, gl_cam, fovy_deg, obj_pos, is_holding, goal_achieved):
    w_sub = 640
    h_sub = 480
    ...
    p_obj = project_point_to_gl_cam(gl_cam, fovy_deg, obj_pos, w_sub, h_sub)
    ...
    draw.text((20, 6), "BILEVEL PLANNING IN SHELF3D: Pick & Place Execution", fill=(255, 255, 255), font=font_large)
    draw.text((20, 30), f"Abstract Plan: [1] PickTargetOperator(robot, cube1)  ->  [2] PlaceTargetOperator(robot, cube1, cupboard_1)", fill=(190, 205, 220), font=font_small)
    draw.text((20, 48), f"Active Operator: {current_op}  |  Step: {step:03d}/{total_steps:03d}", fill=(80, 210, 255), font=font_med)

    # Bottom Banner
    status_text = "GOAL ACHIEVED: cube1 placed safely on cupboard shelf!" if goal_achieved else ("Holding cube1 | Carrying to cupboard" if is_holding else "Approaching & Grasping cube1 from floor")
    status_color = (40, 220, 80) if goal_achieved else ((255, 200, 60) if is_holding else (100, 200, 255))
    
    draw.rectangle([0, total_h - 52, total_w, total_h], fill=(12, 16, 20))
    draw.text((20, total_h - 44), f"Continuous Controller: {current_skill}", fill=(255, 230, 120), font=font_med)
...
def run_shelf_bilevel_rollout(seed=42, output_dir=Path("docs/shelf_videos")):
    ...
    for step, action in enumerate(planned_actions):
        ...
        obj_pos = np.array([float(state.get(cube1, "x")), float(state.get(cube1, "y")), float(state.get(cube1, "z"))])
        ...
        is_in_cupboard = float(obj_pos[0]) > 0.8 and float(obj_pos[2]) > 0.4
        
        current_op = "place_target(robot, cube1, cupboard_1)" if is_holding else "pick_shelf(robot, cube1)"
        current_skill = "TidyBotPlaceShelfController" if is_holding else "TidyBotPickShelfController"

        gl_cam = rc.scn.camera[0]
        goal_done = bool(oc._check_goals())

        annotated = annotate_shelf_frame(
            close_raw, wide_raw, step, total_steps, current_op, current_skill,
            gl_cam, fovy, obj_pos, is_holding, goal_done
        )
    ...
    for s in range(30):
        ...
        goal_done = bool(oc._check_goals())
        annotated = annotate_shelf_frame(
            close_raw, wide_raw, total_steps + s, total_steps, "Goal Evaluation", "Resting in Cupboard",
            gl_cam, fovy, obj_pos, True, goal_done
        )
        frames.append(annotated)
