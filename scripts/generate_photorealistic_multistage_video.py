"""Render high-fidelity photorealistic multi-stage waypoint video on Seed 103:
- scene_bg=True (Photorealistic MimicLabs Lab2 textures, hardwood floor, lighting, soft shadows)
- Clear, unobstructed camera views:
  - Left Panel: Elevated Close-Up Camera (looking at cube & gripper from side-front)
  - Right Panel: Global Scene Camera (wide task_view)
- Explicit Settle / Standstill Stops:
  - Step 1: Move above cube (z+12cm) -> FULL STOP & SETTLE (0.0 m/s)
  - Step 2: Rotate wrist in mid-air to cube yaw -> FULL STOP & SETTLE (0.0 m/s)
  - Step 3: Pure vertical elevator descent -> FULL STOP & SETTLE (0.0 m/s)
  - Step 4: Clamp center of mass & vertical lift
  - Step 5: Move to toss location -> Windup -> Perfect Toss into Goal Bin
- 10 fps (1/2 speed slow-motion)."""
...
class PhotorealisticWaypointPickCubeController(TidyBotPickController):
    STANDOFF = (0.55, 0.0)
    ...
    def reset(self, x, params=None):  # noqa: PLR0917  (overrides TidyBotPickController.reset)
        ...
        for rotation in rotations:
            ...
            try:
                ...
                self._setup_waypoints(plan_x=upright, rotation=rotation)
                return
            except Exception:
                continue
        raise RuntimeError("Failed to plan pick")

    def _setup_waypoints(self, *, plan_x, rotation):
        ...
        s3_traj, s3_dir = _compute_per_joint_profile(
            np.array(q_pre_aligned[:7]), np.array(q_grasp[:7]), v_desc, a_desc
        )

        self._stages = [
            ("Stage 1: Transit Above Cube (z+12cm)", s1_traj, s1_dir, np.array(q_pre_high[:7]), 15),
            (
                "Stage 2: Align Wrist Yaw at Standoff",
                s2_traj,
                s2_dir,
                np.array(q_pre_aligned[:7]),
                12,
    ...
    def step(self):
        ...
        if self._stage_idx < len(self._stages):
            ...
            if not self._settling:
                ...
                # Standstill Settle Phase at Waypoint
                curr = np.array(self._get_current_robot_arm_conf()[:7])
                action = np.zeros(11, dtype=np.float32)
...
def render_photorealistic_video(*, seed=103, output_dir=Path("docs/photorealistic_videos")):
    ...
    pick_ctrl = PhotorealisticWaypointPickCubeController((robot, cube))
    ...
    skills = [
        ("1. Multi-Stage Waypoint Pick", pick_ctrl, None, 200, []),
    ...
    def capture_frame(*, phase_title):
        ...
        d.text(
            (20, 6),
            "MULTI-STAGE WAYPOINT CONTROLLER (With Standstill Settle Stops) [1/2 SPEED]",
            fill=(100, 255, 100),
            font=font_large,
        )
        d.text(
            (20, 28),
            "1. Move to z+12cm & STOP -> 2. Align Wrist Yaw & STOP -> 3. Straight Vertical Descent "
            "& STOP -> 4. Clamp & Toss",
            fill=(180, 220, 180),
            font=font_small,
        )
    ...
    mp4_path = output_dir / f"photorealistic_multi_stage_seed{seed}.mp4"
    gif_path = output_dir / f"photorealistic_multi_stage_seed{seed}.gif"

    imageio.mimsave(mp4_path, frames, fps=10)
    imageio.mimsave(gif_path, frames[::2], fps=5)
    print(f"Saved Photorealistic Multi-Stage Video to {mp4_path} ({len(frames)} frames @ 10 fps)")
