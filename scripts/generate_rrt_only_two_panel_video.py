"""Generate dual-view (Close-Up Cube + Global Scene) video for Full RRT Waypoint Tracking
(NO hardcoded stages).
...
class RRTWaypointPickCubeController(TidyBotPickController):
    ...
    def step(self):
        ...
        if self._dense_plan_idx < len(self._dense_approach_plan):
            ...
            if (
                np.max(np.abs(err)) < 0.025
                or self._dense_plan_idx == len(self._dense_approach_plan) - 1
            ):
...
def render_rrt_video(*, seed=103, output_dir=Path("docs/rrt_videos")):
    ...
    def capture_frame(*, phase_title):
        ...
        d.text(
            (20, 6),
            "PURE RRT MOTION PLANNING (No Hardcoded Stages) [1/2 SPEED SLOW-MOTION]",
            fill=(100, 255, 100),
            font=font_large,
        )
        d.text(
            (20, 28),
            "Executing full collision-free RRT plan directly from PyBullet (RRT routes arm safely "
            "over cube obstacle)",
            fill=(180, 220, 180),
            font=font_small,
        )
        d.text(
            (20, 46),
            f"Active Phase: {phase_title} | Step: {total_step:03d} | Seed: {seed}",
            fill=(80, 210, 255),
            font=font_med,
        )
        ...
        d.text(
            (20, total_h - 35),
            "Controller: RRTWaypointPickCubeController | "
            "Trajectory: Full 31-Waypoint RRT Execution",
            fill=(255, 230, 120),
            font=font_small,
        )
    ...
    for skill_name, ctrl, params, limit, dis_col in skills:
        ...
        for _s in range(limit):
            ...
            state = env.observation_space.devectorize(obs)
            ctrl.observe(state)
            capture_frame(phase_title=skill_name)
            total_step += 1
            if ctrl.terminated():
                break
