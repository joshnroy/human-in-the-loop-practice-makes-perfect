...
one of the registered core.Method implementations through PracticeLoop (--method, a
global flag here since a Method is meant to work across environments). Every flag is
named -- no positional arguments anywhere.

Global flags (--seed, --num-test-tasks, --env, --method, --output-dir, --gif) are
generic runner concepts meaningful for any environment/method. Environment-specific
flags depend on --env's value, and a method's own flags (once --method is chosen)
come from its own add_arguments:
    python -m hitl_pmp.cli --seed 7 --num-test-tasks 5 --env lightswitch --grid-size 3
    python -m hitl_pmp.cli --env lightswitch --method random-skills --num-cycles 10
...
domain- or method-specific knowledge.
"""
...
METHODS = {"random-skills": RandomSkillsCli}
...
class Cli:
    ...
    LightSwitchCli/RandomSkillsCli, which this delegates to once --env/--method
    are known)."""
    ...
    def add_global_arguments(*, parser: argparse.ArgumentParser) -> None:
        parser.add_argument(
            ...
            "--gif: no environment's own add_arguments may redefine them (argparse "
            "itself will raise a clear conflicting-option-string error if one ever "
            "tries).",
        )
        parser.add_argument(
            "--method",
            ...
            "environment-specific flag). Determines which method-specific flags "
            "(e.g. --num-cycles) are valid.",
        )
        parser.add_argument(
            "--seed",
            type=int,
        ...
        parser.add_argument(
            ...
            "episode.mp4 (plus stats.json if --method is set). Disabled (nothing "
            "written) if omitted.",
        )
        parser.add_argument(
            "--gif",
            action="store_true",
            help="If set alongside --output-dir, additionally convert the recorded "
            "episode to <output-dir>/episode.gif. No effect without --output-dir.",
        )

    @staticmethod
    def parse_args(*, argv: list[str] | None = None) -> argparse.Namespace:
        # --env/--method determine which other flags are valid, so they can't all be
        # registered up front. A plain two-pass parse_known_args here would exit
        # early on --help before ever reaching those specific flags (argparse's help
        # action fires mid-scan, during the *first* pass) -- so this discovery pass
        # disables its own help handling and doesn't require --env, purely to learn
        # --env/--method's values without ever printing anything or exiting. The real
        # parser built below always has the full flag set (global + the selected
        # environment's/method's), so it's the one that ever shows --help or reports
        # a genuine "missing --env" error.
