...
class Problem(BaseModel, abc.ABC):
    ...
    def execute_movables_reset(self, *, destination: str | None = None) -> None:
        """The *partial*-reset command: let the human reposition whichever
        non-robot objects this domain considers movable, robot untouched. An optional
        symbolic destination is forwarded unchanged; there is no `goal`/`target_state`
        to build, unlike `execute_human_command`.
        ...
        if destination is None:
            ...
            self.human.execute_movables_reset(env=self.env, destination=destination)
