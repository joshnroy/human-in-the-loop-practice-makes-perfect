...
class TossingRoomSplitIdentitySkills:
    """Lifted skill templates for Tossing Room (split throws, identity representation).

    **Identical to `TossingRoomSplitSkills`** -- same seven skills, same names, same
    arities, same preconditions and effects, same `param_dim`. This file is not where
    this domain differs; the throw representation lives entirely in `environment.py` and
    `tasks.py`. The equivalence is asserted, not assumed, by
    `tests/environments/tossingroomsplitidentity/test_fork_equivalence.py`.

    The rest of this docstring is that domain's, describing the throw split both share.

    **The one difference from `TossingRoomSkills`**:
    ...
    -- which for an item is `(kind, target_force)` -- that classifier can and does transfer
    ...
    _robot: ClassVar[Variable] = Variable(
        name="robot", type=TossingRoomSplitIdentityEnvironment.robot_type
    )
    _trash: ClassVar[Variable] = Variable(
        name="trash", type=TossingRoomSplitIdentityEnvironment.trash_type
    )
    _recycling: ClassVar[Variable] = Variable(
        name="recycling", type=TossingRoomSplitIdentityEnvironment.recycling_type
    )
    _room: ClassVar[Variable] = Variable(
        name="room", type=TossingRoomSplitIdentityEnvironment.room_type
    )
    _from_room: ClassVar[Variable] = Variable(
        name="from_room", type=TossingRoomSplitIdentityEnvironment.room_type
    )
    _to_room: ClassVar[Variable] = Variable(
        name="to_room", type=TossingRoomSplitIdentityEnvironment.room_type
    )
    _trash_bin: ClassVar[Variable] = Variable(
        name="trash_bin", type=TossingRoomSplitIdentityEnvironment.trash_bin_type
    )
    _recycling_bin: ClassVar[Variable] = Variable(
        name="recycling_bin", type=TossingRoomSplitIdentityEnvironment.recycling_bin_type
    )
    _trash_button: ClassVar[Variable] = Variable(
        name="trash_button", type=TossingRoomSplitIdentityEnvironment.trash_button_type
    )
    _recycling_button: ClassVar[Variable] = Variable(
        name="recycling_button", type=TossingRoomSplitIdentityEnvironment.recycling_button_type
    )
    _pile: ClassVar[Variable] = Variable(
        name="pile", type=TossingRoomSplitIdentityEnvironment.pile_type
    )
    ...
    def sample_params(*, ground_skill: GroundSkill, rng: np.random.Generator) -> np.ndarray:
        ...
        Deliberately the SAME base distribution for both throws, and the same one the
        causal arm (`environments/tossingroomsplit`) uses. The experiment compares how
        far each learned sampler moves away from this prior; giving one kind a head start
        here would be exactly the confound to avoid.

        A random draw misses the throw_tolerance window with probability 0.8 on every
        task, identically to the causal arm -- `TossingRoomSplitIdentityTasks` draws
        `target_force` from [0.1, 0.9) precisely so that no task's winning window is
        clipped by the edge of this U(0, 1) band. There is still no single force that
        works everywhere, since the target is redrawn per task; the difference from the
        causal arm is that here the target is an OBSERVED feature (input index 4) rather
        than an unobserved function of two causes, so the sampler's job is to copy a
        column rather than learn a relation."""
    ...
    def compute_action(*, ground_skill: GroundSkill, params: np.ndarray, state: State) -> Action:
        ...
        `TossingRoomSplitIdentityEnvironment`); the kind is carried in `arg` and read off a bound
        object's own `kind` feature, so the two branches of each pair differ only in
        which object they read."""
        skills = TossingRoomSplitIdentitySkills
