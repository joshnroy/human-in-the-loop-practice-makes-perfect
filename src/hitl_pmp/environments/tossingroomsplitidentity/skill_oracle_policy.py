...
class SkillOraclePolicy:
    ...
    *which lifted skill* to use, not just which objects to bind -- `TrashInBin` in the
    ...
    def get_labeled_action(
        *, state: State, env: TossingRoomSplitIdentityEnvironment, goal: Goal
    ) -> LabeledAction:
        rooms = env.get_rooms()
    ...
    def _empty_step(
        *,
        ...
        skill = (
            ...
            else TossingRoomSplitIdentitySkills.PRESS_TRASH
        )
    ...
    def _throw_step(
        *,
        ...
        pickup = (
            ...
            if recycling
            else TossingRoomSplitIdentitySkills.PICKUP_TRASH
        )
        throw = (
            TossingRoomSplitIdentitySkills.THROW_RECYCLING
            if recycling
            else TossingRoomSplitIdentitySkills.THROW_TRASH
        )
        ...
        # In the bin's room, holding the item: throw with the force this item needs.
        # Under the identity representation the oracle is NOT privileged here at all --
        # it reads `target_force` straight out of the State, and every throw sampler sees
        # that same value at index 4 of its own classifier row. In the causal arm the
        # same call additionally needs the five relation coefficients, which no sampler
        # can see; here there is nothing left to know.
    ...
    def _move_toward(
        *, rooms: tuple[Object, ...], robot_room: int, target_room: int
        ...
        ground_skill = GroundSkill(
            ...
            objects=(TossingRoomSplitIdentityEnvironment.robot, rooms[robot_room], rooms[to_room]),
        )
        return ground_skill, np.zeros(0)
