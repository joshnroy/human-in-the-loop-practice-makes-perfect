...
from .environment import TossingRoomSplitIdentityEnvironment  # noqa: E402


class TossingRoomSplitIdentityRenderer(Renderer):
    ...
    def _hand_color(*, holding: int) -> str:
        if holding == TossingRoomSplitIdentityEnvironment.RECYCLING_KIND:
            return TossingRoomSplitIdentityRenderer.recycling_color
        if holding == TossingRoomSplitIdentityEnvironment.TRASH_KIND:
            return TossingRoomSplitIdentityRenderer.trash_color
        return "black"

    @staticmethod
    def _draw_robot(*, ax: Axes, x: float, y: float, holding: int) -> None:
        r = TossingRoomSplitIdentityRenderer
    ...
    def render_frame(*, state: State, env: Environment, label: str | None = None) -> np.ndarray:
        assert isinstance(env, TossingRoomSplitIdentityEnvironment)
        r = TossingRoomSplitIdentityRenderer
