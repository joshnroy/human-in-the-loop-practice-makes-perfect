...
`tests/environments/tossingroomsplitidentity/test_skills.py` asserts a cross-kind grounding is
...
class ItemInBinClassifier:
    ...
    the item. A bin holds at most one item (`TossingRoomSplitIdentityEnvironment.BIN_CAPACITY`)
...
HAND_EMPTY = Predicate(
    name="HandEmpty",
    types=(TossingRoomSplitIdentityEnvironment.robot_type,),
...
TRASH_BIN_EMPTY = Predicate(
    name="TrashBinEmpty",
    types=(TossingRoomSplitIdentityEnvironment.trash_bin_type,),
...
RECYCLING_BIN_EMPTY = Predicate(
    ...
    types=(TossingRoomSplitIdentityEnvironment.recycling_bin_type,),
    holds=lambda state, objects: BinEmptyClassifier.holds(state=state, bin_obj=objects[0]),
)
