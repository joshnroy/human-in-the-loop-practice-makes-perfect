...
def _compute_oracle_action(*, state: State) -> Action:
    ...
    (matches GridRowEnv.simulate's ordering, see environment.py)."""
...
# method/types.py) -- this lambda just adapts that into a call to the keyword-only
# helper above, same pattern as predicates.py's LIGHT_ON.
ORACLE_POLICY: Policy = lambda state: _compute_oracle_action(state=state)  # noqa: E731
