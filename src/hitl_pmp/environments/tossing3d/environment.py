...
class Tossing3DEnvironment(Environment):
    ...
    # hardcoded cube size.
    #
    # The full quaternion and all three extents are carried because `OnGround` is now
    # upstream's face-interchangeable test rather than the old `qx`/`qy`-near-zero one: it
    # composes the measured rotation with the cube's 24-element symmetry group, and it
    # decides whether an object *is* a cube by comparing `bb_x`/`bb_y`/`bb_z`. See
    # `predicates.OnGroundClassifier`.
