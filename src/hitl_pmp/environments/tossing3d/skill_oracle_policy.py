...
class SkillOraclePolicy:
    ...
    def get_labeled_action(
        *,
        ...
        """
        del goal
        # Upstream's classifier, looked up on the abstraction the state carries -- see
        # `predicates.py`. It is strictly stronger than the arithmetic this file used to
        # do for itself: upstream adds a forward-kinematics conjunct, so a gripper that is
        # shut around nothing while the cube happens to be off the floor no longer reads
        # as `Holding` here.
