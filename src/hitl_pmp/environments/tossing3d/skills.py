...
between them**, and derived the pick's standoff and grasp rotation internally so that
**there is nothing for a refiner to backtrack over on the pick**. Both changes move work
out of this package and into upstream's controllers, which is the direction this domain's
rules point: `kinder_backend.py` drives upstream's controllers unmodified, and this
package invents no controller parameter.
...
upstream measured 15/15 scoring throws leaving the cube resting on a face. `OnGround` is
this domain's face-interchangeable test for exactly that reason -- see
`predicates.OnGroundClassifier`, which had to change from "flat on the face it started on"
before this add effect could be honest.
