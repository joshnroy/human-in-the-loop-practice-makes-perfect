...
class SkillOraclePolicy:
    ...
    *which lifted skill* to use, not just which objects to bind -- `TrashInBin` in the
    ...
    def get_labeled_action(
        *, state: State, env: TossingRoomSplitEnvironment, goal: Goal
    ) -> LabeledAction:
        rooms = env.get_rooms()
    ...
    def _empty_step(
        *,
        ...
        skill = (
            ...
            else TossingRoomSplitSkills.PRESS_TRASH
        )
    ...
    def _throw_step(
        *,
        ...
        pickup = (
            ...
            if recycling
            else TossingRoomSplitSkills.PICKUP_TRASH
        )
        throw = (
            TossingRoomSplitSkills.THROW_RECYCLING
            if recycling
            else TossingRoomSplitSkills.THROW_TRASH
        )
    ...
    def _move_toward(
        *, rooms: tuple[Object, ...], robot_room: int, target_room: int
        ...
        ground_skill = GroundSkill(
            ...
            objects=(TossingRoomSplitEnvironment.robot, rooms[robot_room], rooms[to_room]),
        )
        return ground_skill, np.zeros(0)
