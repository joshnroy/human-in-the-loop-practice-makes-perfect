...
class _FakeMetrics(Metrics):
    recorded: ClassVar[list[tuple[int, int, int]]] = []

    @staticmethod
    def record_evaluation(*, num_online_transitions: int, num_solved: int, num_total: int) -> None:
        _FakeMetrics.recorded.append((num_online_transitions, num_solved, num_total))
    ...
    @staticmethod
    def task_training_curve_by_subtask() -> object:
        raise NotImplementedError
...
def test_run_evaluates_once_before_any_cycle_and_once_after_each_cycle() -> None:
    ...
    assert len(_FakeMetrics.recorded) == 4
...
def test_run_records_num_online_transitions_at_each_evaluation_checkpoint() -> None:
    ...
    transitions_recorded = [transitions for transitions, _, _ in _FakeMetrics.recorded]
...
def test_run_evaluates_exactly_num_test_tasks_per_checkpoint() -> None:
    ...
    assert all(num_total == 4 for _, _, num_total in _FakeMetrics.recorded)
