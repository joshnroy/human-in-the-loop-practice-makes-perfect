"""The logic under `analysis/tossing3d_range_and_release_angle.py`'s plotting, all of
which fails silently rather than loudly: the permutation test's exact 2/1024 floor, Holm's
step-down monotonicity, and reset detection off the discrete release step index rather than
...
def test_permutation_p_hits_the_exact_floor_when_every_pair_moves_the_same_way() -> None:
    ...
    the two-sided p is exactly 2/1024."""
...
def test_permutation_floor_sits_above_the_holm_threshold_for_the_committed_grid() -> None:
    """#226's grid steps 37 speeds, so 36 comparisons. If this stops holding, the log's
    "the Holm column cannot fire" caveat has become wrong."""
    n_steps = 36
...
def test_holm_is_monotone_non_decreasing_in_the_sorted_p_values() -> None:
    """Step-down: a larger raw p can never adjust to a smaller one."""
...
def test_a_split_population_resets_on_its_majority_not_on_its_mean() -> None:
    """A mean would slide between the two and register a reset at the *following* speed --
    the artifact that put 150 into circulation when the measured drop is at 145."""
