...
def _recycling_weight(*, task) -> float:
    ...
    more -- see tests/environments/tossingroom/test_throw_representation.py for why."""
    return task.initial_state.get(obj=TossingRoomEnvironment.recycling, feature_name="weight")
...
def test_train_stream_is_unchanged_by_the_fixed_test_composition(
    *,
    ...
    for the throw-representation change. `build_task` now draws FOUR uniforms per task
    (two item weights, two bin distances) where it drew two `target_force`s, so every
    draw after the first shifts and the goal-family sequences move wholesale. That is a
    real change to the training distribution, not noise -- **every Tossing Room number
    produced after this change is measured on different tasks than every number produced
    before it**, exactly as the capacity-1 change was. Any movement in these literals
    from here on is a regression."""
    tasks = _tasks(seed=seed, num_test_tasks=30)
