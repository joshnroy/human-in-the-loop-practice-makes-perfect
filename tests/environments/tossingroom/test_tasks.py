...
def _weight_seed(*, task) -> float:
    ...
    return task.initial_state.get(obj=TossingRoomEnvironment.pile, feature_name="weight_seed")
    ...
    for _ in range(10):
        ...
        assert task.goal.atoms == frozenset({
            RECYCLING_BIN_EMPTY(
                ...
                objects=(TossingRoomEnvironment.recycling_bin,),
            ),
            TRASH_BIN_EMPTY(
                state=task.initial_state,
                objects=(TossingRoomEnvironment.trash_bin,),
            ),
        })
        assert (
            task.initial_state.get(obj=TossingRoomEnvironment.recycling_bin, feature_name="count")
