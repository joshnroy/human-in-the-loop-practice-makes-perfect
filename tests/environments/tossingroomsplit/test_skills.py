...
_TRASH = TossingRoomSplitEnvironment.trash
_RECYCLING = TossingRoomSplitEnvironment.recycling
...
class TestThrowIsTwoSkills:
    ...
    def test_there_are_two_differently_named_throw_skills() -> None:
        names = [skill.name for skill in TossingRoomSplitSkillProvider(env=_ENV).skills()]
    ...
    def test_no_shared_throw_skill_survives() -> None:
        ...
        assert not hasattr(TossingRoomSplitSkills, "THROW")

    @staticmethod
    def test_the_two_throw_skills_are_distinct_objects_with_distinct_content() -> None:
        trash, recycling = (
            TossingRoomSplitSkills.THROW_TRASH,
            TossingRoomSplitSkills.THROW_RECYCLING,
        )
    ...
    def test_each_throw_binds_its_own_item_and_bin_type() -> None:
        ...
        cannot cross them."""
        _robot, trash_item, trash_bin, _room = TossingRoomSplitSkills.THROW_TRASH.parameters
        assert trash_item.type == TossingRoomSplitEnvironment.trash_type
        assert trash_bin.type == TossingRoomSplitEnvironment.trash_bin_type

        _robot, rec_item, rec_bin, _room = TossingRoomSplitSkills.THROW_RECYCLING.parameters
        assert rec_item.type == TossingRoomSplitEnvironment.recycling_type
        assert rec_bin.type == TossingRoomSplitEnvironment.recycling_bin_type

        assert TossingRoomSplitEnvironment.trash_type != TossingRoomSplitEnvironment.recycling_type
        assert (
            TossingRoomSplitEnvironment.trash_bin_type
            != TossingRoomSplitEnvironment.recycling_bin_type
        )
    ...
    def test_throw_recycling_declares_its_own_preconditions_and_effects() -> None:
        skill = TossingRoomSplitSkills.THROW_RECYCLING
        robot, item, bin_var, room = skill.parameters
        assert skill.preconditions == frozenset({
    ...
    def test_no_bin_accepts_item_precondition_is_needed_any_more() -> None:
        ...
        predicate_names = {
            predicate.name for predicate in TossingRoomSplitSkillProvider(env=_ENV).predicates()
        }
...
class TestPickupSplitsWithTheItemTypes:
    ...
    def test_there_are_two_pickups_and_neither_has_continuous_parameters() -> None:
        assert TossingRoomSplitSkills.PICKUP_TRASH.name == "PickupTrash"
        assert TossingRoomSplitSkills.PICKUP_RECYCLING.name == "PickupRecycling"
        assert TossingRoomSplitSkills.PICKUP_TRASH.param_dim == 0
        assert TossingRoomSplitSkills.PICKUP_RECYCLING.param_dim == 0

    @staticmethod
    def test_pickup_trash_keeps_the_pile_room_precondition() -> None:
        skill = TossingRoomSplitSkills.PICKUP_TRASH
...
def test_move_room_requires_a_traversable_step() -> None:
    skill = TossingRoomSplitSkills.MOVE_ROOM
...
class TestPressSplitsWithTheBinAndButtonTypes:
    ...
    def test_there_are_two_presses_and_no_shared_one() -> None:
        ...
        assert not hasattr(TossingRoomSplitSkills, "PRESS")

    @staticmethod
    def test_press_trash_empties_only_the_trash_bin() -> None:
        skill = TossingRoomSplitSkills.PRESS_TRASH
    ...
    def test_press_recycling_empties_only_the_recycling_bin() -> None:
        skill = TossingRoomSplitSkills.PRESS_RECYCLING
        robot, button, room, bin_var, item = skill.parameters
        assert skill.preconditions == frozenset({
            LiftedAtom(predicate=ROBOT_IN_ROOM, variables=(robot, room)),
    ...
    def test_neither_press_needs_ignore_effects_any_more() -> None:
        ...
        assert TossingRoomSplitSkills.PRESS_TRASH.ignore_effects == frozenset()
        assert TossingRoomSplitSkills.PRESS_RECYCLING.ignore_effects == frozenset()
    ...
    def test_a_cross_kind_press_grounding_is_rejected_outright() -> None:
        ...
        with pytest.raises(ValueError, match="type"):
            GroundSkill(
                ...
                objects=(
                    _ROBOT,
                    TossingRoomSplitEnvironment.recycling_button,
                    _ENV.get_rooms()[_ENV.trash_bin_room],
                    _TRASH_BIN,
                    _TRASH,
                ),
...
def test_sample_params_is_empty_for_zero_dim_skills() -> None:
    ...
    params = TossingRoomSplitSkills.sample_params(
...
def test_sample_params_for_each_throw_is_a_single_value_in_unit_interval(
    *, skill_name: str
    ...
    skill = getattr(TossingRoomSplitSkills, skill_name)
    ...
    for _ in range(50):
        params = TossingRoomSplitSkills.sample_params(ground_skill=ground_skill, rng=rng)
...
def test_compute_action_for_pickup_trash_encodes_the_trash_kind() -> None:
    ...
    assert action.tolist() == [
        ...
        float(TossingRoomSplitEnvironment.TRASH_KIND),
        0.0,
    ]
...
def test_compute_action_for_pickup_recycling_encodes_the_recycling_kind() -> None:
    ground_skill = GroundSkill(
        skill=TossingRoomSplitSkills.PICKUP_RECYCLING,
...
def test_compute_action_for_move_room_encodes_the_destination_index() -> None:
    ...
    ground_skill = GroundSkill(
        skill=TossingRoomSplitSkills.MOVE_ROOM, objects=(_ROBOT, rooms[3], rooms[2])
    )
    action = TossingRoomSplitSkills.compute_action(
        ground_skill=ground_skill, params=np.zeros(0), state=_state()
    )
    assert action.tolist() == [float(TossingRoomSplitEnvironment.SKILL_MOVE_ROOM), 2.0, 0.0]
...
def test_compute_action_for_each_throw_uses_the_sampled_force_and_its_own_kind() -> None:
    ...
    assert TossingRoomSplitSkills.compute_action(
        ground_skill=trash_throw, params=np.array([0.42]), state=state
    ).tolist() == [
        float(TossingRoomSplitEnvironment.SKILL_THROW),
        float(TossingRoomSplitEnvironment.TRASH_KIND),
        0.42,
    ]
    assert TossingRoomSplitSkills.compute_action(
...
def test_compute_action_dispatches_by_value_not_identity() -> None:
    rooms = _ENV.get_rooms()
    move = TossingRoomSplitSkills.MOVE_ROOM
    reconstructed = Skill(
        name=move.name,
...
def test_compute_action_rejects_an_unknown_skill() -> None:
    ...
    with pytest.raises(ValueError, match="Unknown skill"):
        TossingRoomSplitSkills.compute_action(
...
class TestGroundingCannotCrossTheTwoKinds:
    ...
    def _throw_task_state():
        ...
        return (
            TossingRoomSplitTasks(env=env, seed=0, forced_goal_type=TossingRoomSplitGoalType.TRASH)
            .sample_test_task()
            .initial_state
        )
...
def test_move_room_cannot_cross_the_ledge_rightward() -> None:
    env = TossingRoomSplitEnvironment()
    state = TossingRoomSplitTasks(env=env, seed=0).sample_test_task().initial_state
