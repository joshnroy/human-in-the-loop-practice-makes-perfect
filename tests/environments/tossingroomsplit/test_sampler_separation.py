...
def _split_method() -> tuple[EesMethod, TossingRoomSplitEnvironment]:
    env = TossingRoomSplitEnvironment()
    method = EesMethod(env=env, skill_provider=TossingRoomSplitSkillProvider(env=env), seed=0)
    return method, env


def _split_throws(*, env: TossingRoomSplitEnvironment) -> tuple[GroundSkill, GroundSkill]:
    rooms = env.get_rooms()
    trash = GroundSkill(
