...
def test_oracle_solves_every_goal_type_on_train_and_test_tasks(
    *, goal_type: TossingRoomSplitGoalType
    ...
    tasks = TossingRoomSplitTasks(env=env, seed=0, forced_goal_type=goal_type)
    problem = TossingRoomSplitProblem(env=env, tasks=tasks)
    method = SkillOracleMethod(env=env, oracle=TossingRoomSplitOracle(env=env))
...
def test_oracle_presses_both_buttons_in_the_only_feasible_order() -> None:
    ...
    env = TossingRoomSplitEnvironment()
    tasks = TossingRoomSplitTasks(env=env, seed=0, forced_goal_type=TossingRoomSplitGoalType.EMPTY)
    problem = TossingRoomSplitProblem(env=env, tasks=tasks)
    method = SkillOracleMethod(env=env, oracle=TossingRoomSplitOracle(env=env))
    for _ in range(10):
        task = tasks.sample_test_task()
...
def test_oracle_never_issues_the_blocked_rightward_ledge_step() -> None:
    ...
    for _ in range(30):
        ...
        for _ in range(problem.max_episode_steps()):
            ...
            if int(round(action[0])) == TossingRoomSplitEnvironment.SKILL_MOVE_ROOM:
...
class TestEesRunsOnThisDomainAndTrainsBothThrowsSeparately:
    ...
    *many* attempts each throw gets is what the experiment measures, and pinning it here
    ...
    def _run() -> EesMethod:
        env = TossingRoomSplitEnvironment()
        tasks = TossingRoomSplitTasks(env=env, seed=0, num_test_tasks=4)
        problem = TossingRoomSplitProblem(env=env, tasks=tasks)
        method = EesMethod(
            env=env,
