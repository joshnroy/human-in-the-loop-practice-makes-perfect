...
_RECYCLING_BIN = TossingRoomSplitEnvironment.recycling_bin
_TRASH_BIN = TossingRoomSplitEnvironment.trash_bin
...
def _fresh_state(*, env: TossingRoomSplitEnvironment):
...
def _pickup(*, kind: int) -> np.ndarray:
    return np.array([float(TossingRoomSplitEnvironment.SKILL_PICKUP), float(kind), 0.0])


def _move(*, to_room: int) -> np.ndarray:
    return np.array([float(TossingRoomSplitEnvironment.SKILL_MOVE_ROOM), float(to_room), 0.0])


def _throw(*, kind: int, force: float) -> np.ndarray:
    return np.array([float(TossingRoomSplitEnvironment.SKILL_THROW), float(kind), force])
...
def _press(*, kind: int) -> np.ndarray:
    ...
    return np.array([float(TossingRoomSplitEnvironment.SKILL_PRESS), float(kind), 0.0])
...
def test_build_initial_state_places_bins_and_buttons_in_their_rooms() -> None:
    ...
    assert state.get(obj=TossingRoomSplitEnvironment.trash, feature_name="weight") == 0.7
    assert state.get(obj=TossingRoomSplitEnvironment.recycling, feature_name="weight") == 0.3
...
def test_build_initial_state_lays_out_room_indices() -> None:
    env = TossingRoomSplitEnvironment(num_rooms=5)
    state = env.build_initial_state(
        trash_weight=1.0, recycling_weight=1.0, trash_bin_distance=2.0, recycling_bin_distance=2.0
    )
    rooms = env.get_rooms()
...
def test_pickup_at_start_room_fills_the_hand() -> None:
    ...
    next_state = env.take_action(action=_pickup(kind=TossingRoomSplitEnvironment.RECYCLING_KIND))
    assert (
        next_state.get(obj=_ROBOT, feature_name="holding")
        == TossingRoomSplitEnvironment.RECYCLING_KIND
    )
...
def test_pickup_is_a_no_op_when_not_in_the_start_room() -> None:
    ...
    env.set_state(state=state)
    next_state = env.take_action(action=_pickup(kind=TossingRoomSplitEnvironment.TRASH_KIND))
...
def test_pickup_is_a_no_op_when_the_hand_is_already_full() -> None:
    ...
    assert (
        next_state.get(obj=_ROBOT, feature_name="holding") == TossingRoomSplitEnvironment.TRASH_KIND
    )
...
def _carry_to_recycling_room(*, env: TossingRoomSplitEnvironment):
    state = env.build_initial_state(
        trash_weight=1.0, recycling_weight=1.0, trash_bin_distance=2.0, recycling_bin_distance=2.0
    )
...
def test_throw_within_tolerance_lands_in_the_bin_and_empties_the_hand() -> None:
    ...
    next_state = env.take_action(
        action=_throw(kind=TossingRoomSplitEnvironment.RECYCLING_KIND, force=0.5)
    )
...
def test_throw_outside_tolerance_consumes_the_item_without_binning_it() -> None:
    ...
    next_state = env.take_action(
        action=_throw(kind=TossingRoomSplitEnvironment.RECYCLING_KIND, force=0.9)
    )
...
def test_a_missed_throw_can_be_retried_only_by_fetching_a_fresh_item() -> None:
    ...
    env = _env()
    _carry_to_recycling_room(env=env)
    env.take_action(action=_throw(kind=TossingRoomSplitEnvironment.RECYCLING_KIND, force=0.9))
    # Picking up in the bin room does nothing: the pile is in the start room.
    after_pickup = env.take_action(
        action=np.array([
            float(TossingRoomSplitEnvironment.SKILL_PICKUP),
            float(TossingRoomSplitEnvironment.RECYCLING_KIND),
            0.0,
        ])
    )
...
def test_press_outside_a_button_room_is_a_no_op() -> None:
    ...
    env.set_state(state=state)
    next_state = env.take_action(action=_press(kind=TossingRoomSplitEnvironment.TRASH_KIND))
    assert next_state.get(obj=_RECYCLING_BIN, feature_name="count") == 1.0
...
class TestBinsHoldAtMostOneItem:
    ...
    @staticmethod
    def _at_the_full_recycling_bin(*, env: TossingRoomSplitEnvironment):
        state = env.build_initial_state(
            trash_weight=1.0,
            recycling_weight=1.0,
            trash_bin_distance=2.0,
    ...
    def test_the_same_throw_lands_once_the_bin_has_been_emptied() -> None:
        ...
        env = _env()
        TestBinsHoldAtMostOneItem._at_the_full_recycling_bin(env=env)
        env.take_action(action=_press(kind=TossingRoomSplitEnvironment.RECYCLING_KIND))
        next_state = env.take_action(
            action=_throw(kind=TossingRoomSplitEnvironment.RECYCLING_KIND, force=0.5)
        )
        assert next_state.get(obj=_RECYCLING_BIN, feature_name="count") == 1.0
    ...
    def test_a_landed_throw_never_pushes_a_bin_past_one() -> None:
        env = _env()
        _carry_to_recycling_room(env=env)
        env.take_action(action=_throw(kind=TossingRoomSplitEnvironment.RECYCLING_KIND, force=0.5))
...
class TestEachBinHasItsOwnButtonBesideIt:
    ...
    @staticmethod
    def _both_bins_full(*, env: TossingRoomSplitEnvironment):
        state = env.build_initial_state(
            trash_weight=1.0,
            recycling_weight=1.0,
            trash_bin_distance=2.0,
    ...
    def test_each_button_sits_in_its_own_bins_room() -> None:
        ...
        assert (
            state.get(obj=TossingRoomSplitEnvironment.trash_button, feature_name="room")
            == env.trash_bin_room
        )
        assert (
            state.get(obj=TossingRoomSplitEnvironment.recycling_button, feature_name="room")
    ...
    def test_pressing_the_recycling_button_empties_only_the_recycling_bin() -> None:
        ...
        state.set(obj=_ROBOT, feature_name="room", feature_val=float(env.recycling_bin_room))
        env.set_state(state=state)
        next_state = env.take_action(action=_press(kind=TossingRoomSplitEnvironment.RECYCLING_KIND))
        assert next_state.get(obj=_RECYCLING_BIN, feature_name="count") == 0.0
        assert next_state.get(obj=_TRASH_BIN, feature_name="count") == 1.0
...
def test_action_space_is_three_dimensional_unbounded_box() -> None:
    assert isinstance(TossingRoomSplitEnvironment.action_space, Box)
    assert TossingRoomSplitEnvironment.action_space.shape == (3,)
    assert np.all(np.isinf(TossingRoomSplitEnvironment.action_space.low))
    assert np.all(np.isinf(TossingRoomSplitEnvironment.action_space.high))


def test_two_way_ledge_is_off_by_default() -> None:
    """The positive control is opt-in. Every result banked before the flag existed was
    run without it, so the default must reproduce the one-way world exactly."""
    assert TossingRoomSplitEnvironment.model_fields["two_way_ledge"].default is False
...
def test_two_way_ledge_clears_the_blocks_right_state_feature() -> None:
    ...
    blocked = _env().build_initial_state(
        trash_weight=1.0, recycling_weight=1.0, trash_bin_distance=2.0, recycling_bin_distance=2.0
    )
    two_way_env = TossingRoomSplitEnvironment(two_way_ledge=True)
    two_way = two_way_env.build_initial_state(
        trash_weight=1.0, recycling_weight=1.0, trash_bin_distance=2.0, recycling_bin_distance=2.0
    )
