"""Offline tests for `KinderBackend`'s sub-step recording switch and its controller seam.
...
def test_the_composed_toss_drives_one_controller_rather_than_three(
    *, monkeypatch: pytest.MonkeyPatch
    ...
    The object 4-tuple is upstream's own `(robot, target, held, barrier)` order -- the bin
    second, not the cube -- so a transposition here would ask the robot to throw the bin
    at the cube."""
    backend, calls = _spy_run_controller(monkeypatch=monkeypatch)
