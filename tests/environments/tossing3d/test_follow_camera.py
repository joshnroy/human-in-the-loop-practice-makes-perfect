...
from hitl_pmp.environments.tossing3d.kinder_backend import (
    KinderBackend,
    interaction_camera_position,
)
...
def test_camera_follows_base_without_accumulating_motion(*, fail):
    ...
    expected = interaction_camera_position(
        points=np.array([[3, 2, 0.7], [4, 2, 3], [6, 2, 0]]),
        original_position=original[0],
        original_target=np.array([1, 0, 0]),
        vertical_fov=45,
        aspect=640 / 480,
    )
...
def test_fit_contains_padded_bounds_during_high_and_far_tosses(*, aspect):
    points = np.array([[0, 0, 0.7], [8, -5, 6], [10, 0, 0]])
    position = interaction_camera_position(
        points=points,
        original_position=np.array([1, -1.8, 5.5]),
        original_target=np.array([1, 0, 0]),
        vertical_fov=65,
        aspect=aspect,
    )
...
def test_replay_keeps_live_static_colliders_when_background_renumbers_ids() -> None:
    ...
    class Snapshot(dict):
        def __init__(self, data, type_features):  # noqa: PLR0917
            super().__init__(data)
            self.type_features = type_features

    cube = NamedObject(name="cube_0")
    collider = NamedObject(name="collider:cuboid_barrier:2")
    backend = KinderBackend()
    backend._state = Snapshot({cube: np.array([0.0]), collider: np.array([2.0])}, {})
    restored = backend.plain_to_snapshot(
        plain={"cube_0": [1.0], "collider:cuboid_barrier:9": [99.0]}
    )
    np.testing.assert_array_equal(restored[cube], [1.0])
    np.testing.assert_array_equal(restored[collider], [2.0])
    assert len(restored) == 2
    with pytest.raises(KeyError):
        backend.plain_to_snapshot(plain={"unknown_movable": [0.0]})
