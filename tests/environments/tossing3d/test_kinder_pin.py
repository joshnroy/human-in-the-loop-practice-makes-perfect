...
def test_the_pick_really_takes_no_parameters() -> None:
    """`param_dim=0` on this side is only honest if upstream's own sampler agrees. It
    returns `tuple()`, which is why `run_pick_cube` passes `params=None` rather than an
    empty array. Called on the class, so no controller is constructed and no PyBullet
    client is opened."""
    from kinder_models.dynamic3d.tossing.parameterized_skills import PickCubeController

    drawn = PickCubeController.sample_parameters(PickCubeController, None, np.random.default_rng(0))
    assert drawn == ()
