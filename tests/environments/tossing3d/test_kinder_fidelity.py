...
def test_the_closed_on_nothing_state_is_absorbing_through_the_production_path() -> None:
    """The defect, read back through `KinderBackend.abstract_atoms` on a real scene
    rather than asserted from a hand-built atom set.

    A gripper commanded shut with the cube still flat on the floor yields **neither**
    `HandEmpty` nor `Holding`. `Pick` needs the first; `MoveToThrowPose` and `Toss` need
    the second. So the applicable set is empty, and nothing the robot can do changes that
    -- which is what "absorbing" means here, and why a recovery operator has to exist.
    """
    env = _env()
    try:
        env.reset_to_seed(seed=CANONICAL_SEED)
        ...
        snapshot = backend.snapshot()
        robot = snapshot.get_object_from_name(backend.robot_name)
        # Commanded shut. The cube is untouched, so it is still on the floor.
        snapshot.set(robot, "pos_gripper", 1.0)

        atoms = backend.abstract_atoms(state=snapshot)
        assert ("HandEmpty", ("robot",)) not in atoms
        assert ("Holding", ("robot", "cube_0")) not in atoms
        # The predicate that names the gap, and the one `OpenGripper` keys off.
        assert ("GripperCommandedClosed", ("robot",)) in atoms
        assert ("OnGround", ("cube_0",)) in atoms
    finally:
        env.close()


def test_open_gripper_actually_reopens_a_gripper_closed_on_nothing() -> None:
    """The recovery, executed. Drives upstream's `close_gripper` on a scene where the
    robot is holding nothing, confirms the state really is the absorbing one, then runs
    `KinderBackend.run_open_gripper` and confirms `HandEmpty` is back.

    This is the half a symbolic test cannot reach: that the operator's add effect is
    something the paired controller genuinely achieves.
    """
    env = _env()
    try:
        env.reset_to_seed(seed=CANONICAL_SEED)
        ...
        closed = backend.run_controller(
            module="tossing",
            key="close_gripper",
            object_names=(backend.robot_name,),
            params=None,
            limit=backend.gripper_step_limit,
        )
        assert closed.terminated, closed.error

        stuck = backend.abstract_atoms()
        assert ("GripperCommandedClosed", ("robot",)) in stuck
        assert ("HandEmpty", ("robot",)) not in stuck
        assert ("Holding", ("robot", "cube_0")) not in stuck

        reopened = backend.run_open_gripper()
        assert reopened.terminated, reopened.error

        recovered = backend.abstract_atoms()
        assert ("HandEmpty", ("robot",)) in recovered
        assert ("GripperCommandedClosed", ("robot",)) not in recovered
        # The cube never moved, so `Pick` is applicable again -- the state is escaped
        # rather than merely relabelled.
        assert ("OnGround", ("cube_0",)) in recovered
        assert ("MovableIsDownX", ("cube_0", "cuboid_barrier")) in recovered
    finally:
        env.close()
