...
from hitl_pmp.environments.tossingroomsplitidentity.skill_provider import (
    TossingRoomSplitIdentityOracle,
...
def test_oracle_solves_every_goal_type_on_train_and_test_tasks(
    *, goal_type: TossingRoomSplitIdentityGoalType
    ...
    tasks = TossingRoomSplitIdentityTasks(env=env, seed=0, forced_goal_type=goal_type)
    problem = TossingRoomSplitIdentityProblem(env=env, tasks=tasks)
    method = SkillOracleMethod(env=env, oracle=TossingRoomSplitIdentityOracle(env=env))
...
def test_oracle_solves_the_default_mixed_distribution() -> None:
    env = TossingRoomSplitIdentityEnvironment()
    tasks = TossingRoomSplitIdentityTasks(env=env, seed=0)
    problem = TossingRoomSplitIdentityProblem(env=env, tasks=tasks)
    method = SkillOracleMethod(env=env, oracle=TossingRoomSplitIdentityOracle(env=env))
    solved = 0
    for _ in range(30):
...
def test_oracle_presses_both_buttons_in_the_only_feasible_order() -> None:
    ...
    tasks = TossingRoomSplitIdentityTasks(
        env=env, seed=0, forced_goal_type=TossingRoomSplitIdentityGoalType.EMPTY
    )
    problem = TossingRoomSplitIdentityProblem(env=env, tasks=tasks)
    method = SkillOracleMethod(env=env, oracle=TossingRoomSplitIdentityOracle(env=env))
    for _ in range(10):
        ...
        for _ in range(problem.max_episode_steps()):
            ...
            if int(round(action[0])) == TossingRoomSplitIdentityEnvironment.SKILL_PRESS:
...
def test_oracle_never_issues_the_blocked_rightward_ledge_step() -> None:
    ...
    for _ in range(30):
        ...
        for _ in range(problem.max_episode_steps()):
            ...
            if int(round(action[0])) == TossingRoomSplitIdentityEnvironment.SKILL_MOVE_ROOM:
...
class TestEesRunsOnThisDomainAndTrainsBothThrowsSeparately:
    ...
    measurement. The cycle count is load-bearing for NON-VACUITY rather than for coverage:
    the assertions below are only meaningful if both throws were actually practiced, and
    which families the first few practice tasks belong to is a function of the training
    stream. This arm's `build_task` draws TWO uniforms per task (one target force per
    item) against the causal arm's four, so the two arms consume `train_rng` at different
    rates and their practice-task sequences diverge -- 4 cycles is what reaches a
    `ThrowRecycling` here (seed 0 draws trash/recycling/trash/empty; see
    tests/environments/tossingroomsplitidentity/test_tasks.py, which pins that sequence).
    ...
    *many* attempts each throw gets is what the experiment measures, and pinning it here
    ...
    def _run() -> EesMethod:
        env = TossingRoomSplitIdentityEnvironment()
        tasks = TossingRoomSplitIdentityTasks(env=env, seed=0, num_test_tasks=4)
        problem = TossingRoomSplitIdentityProblem(env=env, tasks=tasks)
        method = EesMethod(
            env=env,
