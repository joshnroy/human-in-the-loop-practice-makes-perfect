...
_RECYCLING_BIN = TossingRoomSplitIdentityEnvironment.recycling_bin
_TRASH_BIN = TossingRoomSplitIdentityEnvironment.trash_bin
...
def _fresh_state(*, env: TossingRoomSplitIdentityEnvironment):
...
def _pickup(*, kind: int) -> np.ndarray:
    return np.array([float(TossingRoomSplitIdentityEnvironment.SKILL_PICKUP), float(kind), 0.0])


def _move(*, to_room: int) -> np.ndarray:
    return np.array([
        float(TossingRoomSplitIdentityEnvironment.SKILL_MOVE_ROOM),
        float(to_room),
        0.0,
    ])
...
def _throw(*, kind: int, force: float) -> np.ndarray:
    return np.array([float(TossingRoomSplitIdentityEnvironment.SKILL_THROW), float(kind), force])
...
def _press(*, kind: int) -> np.ndarray:
    ...
    return np.array([float(TossingRoomSplitIdentityEnvironment.SKILL_PRESS), float(kind), 0.0])
...
def test_build_initial_state_places_bins_and_buttons_in_their_rooms() -> None:
    ...
    # THE ANSWER lands on the item, per kind and per task: the required force IS
    # `item.target_force` here, so each throw reads its own answer off the object its own
    # skill binds, and the two kinds' forces are independent in the same state.
    assert (
        state.get(obj=TossingRoomSplitIdentityEnvironment.trash, feature_name="target_force") == 0.7
    )
    assert (
        state.get(obj=TossingRoomSplitIdentityEnvironment.recycling, feature_name="target_force")
        == 0.3
    )
    # A bin carries NO throw_distance: it contributes nothing to the required force under
    # this representation, so the causal arm's per-task distance feature is dropped rather
    # than kept as a decorative column. Asserted rather than assumed, because a leftover
    # column would silently widen every throw sampler's input row.
    assert _TRASH_BIN.type.feature_names == ("count", "room", "kind")
    assert _RECYCLING_BIN.type.feature_names == ("count", "room", "kind")


def test_build_initial_state_lays_out_room_indices() -> None:
    env = TossingRoomSplitIdentityEnvironment(num_rooms=5)
    state = env.build_initial_state(
        trash_target_force=0.5,
        recycling_target_force=0.5,
    )
...
def test_pickup_at_start_room_fills_the_hand() -> None:
    ...
    next_state = env.take_action(
        action=_pickup(kind=TossingRoomSplitIdentityEnvironment.RECYCLING_KIND)
    )
    assert (
        next_state.get(obj=_ROBOT, feature_name="holding")
        == TossingRoomSplitIdentityEnvironment.RECYCLING_KIND
    )
...
def test_pickup_is_a_no_op_when_not_in_the_start_room() -> None:
    ...
    env.set_state(state=state)
    next_state = env.take_action(
        action=_pickup(kind=TossingRoomSplitIdentityEnvironment.TRASH_KIND)
    )
...
def test_pickup_is_a_no_op_when_the_hand_is_already_full() -> None:
    ...
    assert (
        next_state.get(obj=_ROBOT, feature_name="holding")
        == TossingRoomSplitIdentityEnvironment.TRASH_KIND
    )
...
def _carry_to_recycling_room(*, env: TossingRoomSplitIdentityEnvironment):
    state = env.build_initial_state(
        trash_target_force=0.5,
        recycling_target_force=0.5,
    )
...
def test_throw_within_tolerance_lands_in_the_bin_and_empties_the_hand() -> None:
    ...
    next_state = env.take_action(
        action=_throw(kind=TossingRoomSplitIdentityEnvironment.RECYCLING_KIND, force=0.5)
    )
...
def test_throw_outside_tolerance_consumes_the_item_without_binning_it() -> None:
    ...
    next_state = env.take_action(
        action=_throw(kind=TossingRoomSplitIdentityEnvironment.RECYCLING_KIND, force=0.9)
    )
...
def test_a_missed_throw_can_be_retried_only_by_fetching_a_fresh_item() -> None:
    ...
    after_pickup = env.take_action(
        action=np.array([
            float(TossingRoomSplitIdentityEnvironment.SKILL_PICKUP),
            float(TossingRoomSplitIdentityEnvironment.RECYCLING_KIND),
            0.0,
        ])
    )
...
def test_press_outside_a_button_room_is_a_no_op() -> None:
    ...
    env.set_state(state=state)
    next_state = env.take_action(action=_press(kind=TossingRoomSplitIdentityEnvironment.TRASH_KIND))
    assert next_state.get(obj=_RECYCLING_BIN, feature_name="count") == 1.0
    assert next_state.get(obj=_TRASH_BIN, feature_name="count") == 1.0


class TestBinsHoldAtMostOneItem:
    """Capacity 1 is the whole point of the bin redesign, and it is ported here
    unchanged from Tossing Room: with the count provably 0 at throw time, the in-bin
    predicate goes false -> true exactly once per throw, so EES's
    ...
    BOTH `ThrowTrash` and `ThrowRecycling` carry the matching bin-empty precondition so
    each symbolic model stays exactly as strong as this guard."""

    @staticmethod
    def _at_the_full_recycling_bin(*, env: TossingRoomSplitIdentityEnvironment):
        state = env.build_initial_state(
            trash_target_force=0.5,
            recycling_target_force=0.5,
            recycling_count=1,
    ...
    def test_the_same_throw_lands_once_the_bin_has_been_emptied() -> None:
        ...
        env = _env()
        TestBinsHoldAtMostOneItem._at_the_full_recycling_bin(env=env)
        env.take_action(action=_press(kind=TossingRoomSplitIdentityEnvironment.RECYCLING_KIND))
        next_state = env.take_action(
            action=_throw(kind=TossingRoomSplitIdentityEnvironment.RECYCLING_KIND, force=0.5)
        )
        assert next_state.get(obj=_RECYCLING_BIN, feature_name="count") == 1.0
...
class TestEachBinHasItsOwnButtonBesideIt:
    ...
    universal delete no per-item effect could express); with one button per bin the
    delete becomes an ordinary `delete_effect` on each of `PressTrash`/
    `PressRecycling`."""

    @staticmethod
    def _both_bins_full(*, env: TossingRoomSplitIdentityEnvironment):
        state = env.build_initial_state(
            trash_target_force=0.5,
            recycling_target_force=0.5,
            recycling_count=1,
    ...
    def test_each_button_sits_in_its_own_bins_room() -> None:
        ...
        assert (
            state.get(obj=TossingRoomSplitIdentityEnvironment.trash_button, feature_name="room")
            == env.trash_bin_room
        )
        assert (
            state.get(obj=TossingRoomSplitIdentityEnvironment.recycling_button, feature_name="room")
    ...
    def test_pressing_the_trash_button_empties_only_the_trash_bin() -> None:
        ...
        env.set_state(state=state)
        next_state = env.take_action(
            action=_press(kind=TossingRoomSplitIdentityEnvironment.TRASH_KIND)
        )
    ...
    def test_pressing_the_recycling_button_empties_only_the_recycling_bin() -> None:
        ...
        env.set_state(state=state)
        next_state = env.take_action(
            action=_press(kind=TossingRoomSplitIdentityEnvironment.RECYCLING_KIND)
        )
        assert next_state.get(obj=_RECYCLING_BIN, feature_name="count") == 0.0
        assert next_state.get(obj=_TRASH_BIN, feature_name="count") == 1.0
...
def test_action_space_is_three_dimensional_unbounded_box() -> None:
    assert isinstance(TossingRoomSplitIdentityEnvironment.action_space, Box)
    assert TossingRoomSplitIdentityEnvironment.action_space.shape == (3,)
    assert np.all(np.isinf(TossingRoomSplitIdentityEnvironment.action_space.low))
    assert np.all(np.isinf(TossingRoomSplitIdentityEnvironment.action_space.high))
