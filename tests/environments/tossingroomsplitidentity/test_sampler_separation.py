...
def _split_method() -> tuple[EesMethod, TossingRoomSplitIdentityEnvironment]:
    env = TossingRoomSplitIdentityEnvironment()
    method = EesMethod(
        env=env, skill_provider=TossingRoomSplitIdentitySkillProvider(env=env), seed=0
    )
    return method, env


def _split_throws(*, env: TossingRoomSplitIdentityEnvironment) -> tuple[GroundSkill, GroundSkill]:
    rooms = env.get_rooms()
    trash = GroundSkill(
...
class TestTheTwoThrowSamplersAreSeparate:
    ...
    def test_each_sampler_holds_exactly_its_own_rows() -> None:
        ...
        # The item's target_force feature -- which under this representation IS the
        # required throw force, sitting at index 4 of the row -- differs between the two
        # kinds in this state, so a leaked row is identifiable by content, not just by
        # count.
...
class TestTheControl:
    ...
    def test_the_unsplit_tossing_room_shares_one_sampler() -> None:
        ...
        rooms = env.get_rooms()
        # NOTE: this control runs against `environments/tossingroom`, the *unsplit*
        # domain, which is on the CAUSAL representation -- hence the weight/distance
        # keywords here rather than this arm's target_force. That is deliberate: what the
        # control isolates is the effect of the skill NAME on sampler keying, which is
        # orthogonal to the throw representation, so the causal-arm control is the right
        # one for this arm too. The two kinds differ only in item weight, so a pooled row
        # is identifiable by content rather than only by count.
