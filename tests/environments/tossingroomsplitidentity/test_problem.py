...
def _no_op_action(*, state) -> LabeledAction:
    ...
    return LabeledAction(
        action=np.array([TossingRoomSplitIdentityEnvironment.SKILL_PRESS, 0.0, 0.0]),
...
def _build_problem(
    *, num_rooms: int = 7, goal_type: TossingRoomSplitIdentityGoalType | None = None
):
    env = TossingRoomSplitIdentityEnvironment(num_rooms=num_rooms)
    tasks = TossingRoomSplitIdentityTasks(env=env, forced_goal_type=goal_type)
    return TossingRoomSplitIdentityProblem(env=env, tasks=tasks)


def _oracle_policy(*, problem, task) -> Policy:
    return SkillOracleMethod(
        env=problem.env, oracle=TossingRoomSplitIdentityOracle(env=problem.env)
...
def test_problem_requires_its_own_domains_types() -> None:
    with pytest.raises(ValueError, match="env"):
        TossingRoomSplitIdentityProblem(
            ...
            tasks=TossingRoomSplitIdentityTasks(env=TossingRoomSplitIdentityEnvironment()),
        )
...
def test_max_episode_steps_grows_when_a_bin_moves_further_away() -> None:
    env = TossingRoomSplitIdentityEnvironment(num_rooms=12, trash_bin_room=11)
    problem = TossingRoomSplitIdentityProblem(env=env, tasks=TossingRoomSplitIdentityTasks(env=env))
...
def test_run_task_episode_solves_a_recycling_task_with_the_oracle() -> None:
    problem = _build_problem(goal_type=TossingRoomSplitIdentityGoalType.RECYCLING)
...
def test_run_task_episode_records_a_frame_per_step_with_a_renderer() -> None:
    ...
    solved, frames = problem.run_task_episode(
        ...
        renderer=TossingRoomSplitIdentityRenderer,
    )
    assert solved is True
...
def test_run_task_episode_sets_env_state_from_the_task_initial_state() -> None:
    ...
    assert problem.env.get_current_state().get(
        obj=TossingRoomSplitIdentityEnvironment.robot, feature_name="room"
    ) == task.initial_state.get(obj=TossingRoomSplitIdentityEnvironment.robot, feature_name="room")
