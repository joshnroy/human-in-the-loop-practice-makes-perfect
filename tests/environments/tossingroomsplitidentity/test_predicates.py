...
_TRASH_BUTTON = TossingRoomSplitIdentityEnvironment.trash_button
_RECYCLING_BUTTON = TossingRoomSplitIdentityEnvironment.recycling_button
...
def test_each_buttons_declared_type_ties_it_to_exactly_one_bins_press() -> None:
    ...
    assert TRASH_BUTTON_IN_ROOM.types[0] == TossingRoomSplitIdentityEnvironment.trash_button_type
    assert (
...
def test_predicates_declare_their_types_and_names() -> None:
    ...
    assert TRASH_BIN_EMPTY.types == (TossingRoomSplitIdentityEnvironment.trash_bin_type,)
    assert RECYCLING_BIN_EMPTY.types == (TossingRoomSplitIdentityEnvironment.recycling_bin_type,)
...
class TestTheSplitPredicatesCannotBeAppliedAcrossKinds:
    ...
    **Not** in `GroundAtom`: it is deliberately un-validated (`core/problem/tasks/
    ...
    def test_the_abstraction_contains_no_cross_kind_atom() -> None:
        ...
        provider = TossingRoomSplitIdentitySkillProvider(env=_ENV)
