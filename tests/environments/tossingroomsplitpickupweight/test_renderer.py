...
def test_render_frame_annotates_the_ledge_differently_when_it_is_two_way() -> None:
    ...
    one_way = TossingRoomSplitPickupWeightEnvironment()
    two_way = TossingRoomSplitPickupWeightEnvironment(two_way_ledge=True)
    one_way_frame = TossingRoomSplitPickupWeightRenderer.render_frame(
        state=_state(env=one_way), env=one_way
    )
    two_way_frame = TossingRoomSplitPickupWeightRenderer.render_frame(
        state=_state(env=two_way), env=two_way
    )
