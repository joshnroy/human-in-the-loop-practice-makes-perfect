...
# get_cells/build_initial_state/set_state/take_action read instance-level
# grid_size now -- a bare default-constructed instance is all these tests need
# (nothing here overrides grid_size).
...
def test_move_robot_end_to_end_reaches_the_target_cell() -> None:
    _ENV.set_state(state=_ENV.build_initial_state(light_level=0.0, light_target=0.5))
    cells = _ENV.get_cells()
    ground_skill = GroundSkill(
        skill=LightSwitchSkills.MOVE_ROBOT,
    ...
    next_state = _ENV.take_action(action=action)
